...
public class DatasetParser {
    public static CallRecord[] parseCallRecords(String json) throws Exception {
        JsonNode root = Json.MAPPER.readTree(json);

        if (root.isArray()) {
            return Json.MAPPER.treeToValue(root, CallRecord[].class);
        }

        if (root.isObject()) {
            JsonNode arr = root.get("callRecords"); // ✅ correct field name
            if (arr != null && arr.isArray()) {
                return Json.MAPPER.treeToValue(arr, CallRecord[].class);
            }
